...
class Battle():
    ...
    def create_monster(self, monster, index, pos_index, entity):
        ...
        # Load monster and outline frames
        frames = self.monster_frames['monsters'][monster.name]
        outline_frames = self.monster_frames['outlines'][monster.name]

        # Flip frames for player monsters
        if entity == 'player':
            pos = list(BATTLE_POSITIONS['left'].values())[pos_index]
            groups = (self.battle_sprites, self.player_sprites)

            # Flip monster frames horizontally
            frames = {
                state: [pygame.transform.flip(frame, True, False) for frame in frame_list]
                for state, frame_list in frames.items()
            }

            # Flip outline frames horizontally
            outline_frames = {
                state: [pygame.transform.flip(frame, True, False) for frame in frame_list]
                for state, frame_list in outline_frames.items()
            }
        ...
        # Create the monster sprite and its outline
    ...
    def check_active(self):
        for monster_sprite in self.player_sprites.sprites() + self.opponent_sprites.sprites():
            if monster_sprite.monster.initative >= 100:
    ...
    def apple_attack(self, target_sprite, attack, amount):
            ...
            if monster_sprite.monster.health <= 0:
                if self.player_sprites in monster_sprite.groups():
                    ...
                    if available_monster:
                        new_monster_data = [(monster, index, monster_sprite.pos_index, 'player') for index, monster in available_monster]
                    else:
                    ...
                    monster_sprite.kill()
                    xp_amount = monster_sprite.monster.levek * 100 / len(self.player_sprites)
    ...
    def opponent_attack(self):
        ability = (self.current_monster.monster.get_abilities())
        ...
        self.current_monster.activate_attack(random_target)
